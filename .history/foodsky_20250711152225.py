...
def generate_all_dishes_reasons(dishes_info, meal_needs, weights, user_info, meal_type):
    ...
    # 准备提示词
    prompt = f"""
你是一位专业的营养师，以下是餐单中的14道菜品信息，请结合用户基本信息，用户单餐营养需求，菜品信息列表，每道菜的权重为每道菜品生成推荐或者不推荐理由：
...
1. 判断每道菜是推荐食用还是不推荐食用，必须明确是食用还是不食用
2. 权重是系统根据营养均衡度与适配性评估得出的指标，高表示推荐，低表示不推荐
3. 推荐食用的菜品营养范围总和不能超过用户单餐营养需求
