...
### 用户单餐营养需求:{format_nutrition_table(meal_needs, is_range=True)}

## 菜品信息列表:{format_dishes_table(dishes_info)}

## 任务要求
1. 结合用户基本信息，用户单餐营养需求，以及菜品信息列表，判断每道菜是否推荐食用
2. 结合权重解释原因（权重是系统根据营养均衡度与适配性评估得出的指标，取值范围为 0-1，越高表示越推荐。）
...
"""
    ...
    try:
        ...
        app_log.debug(f"大模型返回内容: {content[:200]}...")
        
        
        # 尝试解析JSON
try:
    response_content = response.choices[0].message.content
    app_log.debug(f"大模型返回原始内容: {response_content}")  # 记录原始响应
    return json.loads(response_content)
except json.JSONDecodeError:
    app_log.error(f"无效的JSON格式: {response_content}")
    return {}
