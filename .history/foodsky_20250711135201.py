...
def generate_all_dishes_reasons(dishes_info, meal_needs, weights, user_info, meal_type):
    ...
    # 准备提示词
    prompt = f"""
你是一位专业的营养师，正在为用户设计一份完整的{meal_type}餐单。以下是餐单中的14道菜品信息，请为每道菜品生成推荐或者不推荐理由：
...
1. 结合用户基本信息，用户单餐营养需求，菜品信息列表，和每道菜的权重，判断每道菜是推荐食用还是不推荐食用
2. 其中权重是系统根据营养均衡度与适配性评估得出的指标，越高表示越推荐，越低表示不推荐
3. 最终所有推荐食用的菜品营养范围总和不能超过用户单餐营养需求

## 所有的输出格式使用JSON格式输出
## 输出示例
{{
    "番茄炒蛋": "富含优质蛋白和维生素C，脂肪含量适中。推荐权重0.85，营养均衡且符合需求，建议食用。",
    "红烧肉": "高脂肪高热量，蛋白质含量中等。推荐权重0.05，建议本餐不食用红烧肉，避免过量摄入脂肪。"
    ...
    "总体解释"："根据用户的营养需求和菜品信息，系统根据营养均衡度与适配性评估得出了每道菜的推荐权重。推荐权重越高表示越推荐，越低表示不推荐。"
}}
"""
    ...
    try:
        ...
        try:
            ...
            app_log.error(f"大模型返回内容不是合法JSON: {response_content}")
            app_log.error(f"解析失败: {str(e)}")
...
def format_dishes_table(dishes_info):
    return "\n".join([
        f"菜品: {dish['name']}, 权重: {dish['weight']:.2f}, 营养: [{format_nutrition_table(dish['nutrition'], is_range=False)}]"
...
def format_nutrition_table(nutrition_data, is_range=False):
    ...
    for nutrient in key_nutrients:
        ...
        if is_range and isinstance(value, (tuple, list)):
            # 范围值处理 - 分别格式化最小值和最大值
            try:
                min_val = float(value[0])
                max_val = float(value[1])
                display_value = f"{min_val:.1f}-{max_val:.1f}"
            ...
            try:
                num_val = float(value)
                display_value = f"{num_val:.1f}"
            except (TypeError, ValueError):
                display_value = str(value)
        
        results.append(f"{nutrient}:{display_value}")
