...
def parse_diet_data(data):
    ...
    return f"{data['flag']}天内容，一位{age}岁{gender}性（身高{height}cm，体重{weight}kg，活动水平{activity}在{meal_type}食用了{dishes_str}。"
    ...
    completion = client.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "user", "content": f"""
                            ...
                            请结合推荐与实际摄入量，针对该用户饮食状况，用20字左右给出合理的健康膳食建议。
             """}
        ]
    )
    return completion.choices[0].message.content

def dict_to_recommendation_text(nutrient_dict):
    parts = []
    for nutrient, value in nutrient_dict.items():
        parts.append(f"{nutrient}为{value}")
    sentence = "推荐摄入量包括：" + "，".join(parts) + "。"
    return sentence

def dict_to_personal_text(nutrient_dict):
    parts = []
    for nutrient, value in nutrient_dict.items():
        parts.append(f"{nutrient}为{value}")
    sentence = "当前摄入量是：" + "，".join(parts) + "。"
    return sentence

#app_log = setup_logger("logger", "logs/app_log.log")
#@app.post('/getNutrition')
async def getNutrition(json_data):  
    ...
    if flag == 0:
        ...
        recommend_nutrition = dict_to_recommendation_text(recommend_nutrition_range)
        total_nutrition_all_recipe_personal = dict_to_personal_text(total_nutrition_all_recipe)
        illness = "无"
        if data["info"]["疾病情况"]:
            illness = data["info"]["疾病情况"]
        output = await ask(illness,info_data,recommend_nutrition,total_nutrition_all_recipe_personal)
...
if __name__ == "__main__":
    ...
    result = asyncio.run(getNutrition(test_data))
    print(result)
