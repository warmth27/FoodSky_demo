...
#! 生成所有菜品的推荐理由 Foodsky
def generate_all_dishes_reasons(dishes_info, meal_needs, weights):
    """
    批量生成所有菜品的推荐理由
    ...
    # 准备提示词
    prompt = f"""
你是一位专业的营养师，请根据以下信息为14道菜品生成推荐理由：
...
## 任务要求
1. 为每道菜品生成独立的推荐理由（30-50字）
2. 分析菜品的主要营养特点（如高蛋白、低脂肪等）
3. 结合推荐权重解释原因（权重高说明营养均衡，权重低说明营养不均衡）
4. 给出具体食用建议（如"作为主菜"、"少量搭配"等）
5. 语言简洁专业，适合普通用户理解

## 输出格式
使用JSON格式输出，包含菜品名称和推荐理由，例如：
{{
    "番茄炒蛋": "富含优质蛋白和维生素C，脂肪含量适中。推荐权重0.85，营养均衡且符合需求，建议作为午餐主菜食用。",
    "红烧肉": "高脂肪高热量，蛋白质含量中等。推荐权重0.45，建议少量搭配蔬菜食用，避免过量摄入脂肪。",
    ...
}}
"""
...
def format_dishes_table(dishes_info):
    ...
    for dish in dishes_info:
        table.append(f"### {dish['name']}")
        table.append(f"- 推荐权重: {dish['weight']:.2f}")
        table.append("- 营养数据 (每100克):")
        table.append(format_nutrition_table(dish['nutrition']))
    return "\n".join(table)

def format_nutrition_table(nutrition_data, is_range=False):
    """格式化营养数据为表格"""
    key_nutrients = ["能量", "蛋白质", "脂肪", "碳水化合物", "钠", "钙", "维生素C"]
    table = []
    ...
    for nutrient in key_nutrients:
        ...
        if is_range:
            ...
            if isinstance(value, tuple):
                display_value = f"{value[0]:.1f}-{value[1]:.1f}"
            else:
        ...
        table.append(f"  - {nutrient}: {display_value}")
    
    return "\n".join(table)

def generate_simple_reasons(dishes_info, weights):
    """批量生成简单的推荐理由（大模型不可用时使用）"""
    reasons = {}
    for i, dish in enumerate(dishes_info):
        name = dish['name']
        ...
        # 分析营养特点
        features = []
        if nutrition.get('蛋白质', 0) > 15:
            features.append("高蛋白")
        if nutrition.get('脂肪', 0) > 20:
            features.append("高脂肪")
        if nutrition.get('碳水化合物', 0) > 40:
            features.append("高碳水")
        if nutrition.get('钠', 0) > 500:
            features.append("高钠")
        ...
        # 组合理由
        if features:
            reasons[name] = f"{name}是{','.join(features)}菜品，{advice}。"
        else:
            reasons[name] = f"{name}营养均衡，{advice}。"
...
class DishOptimizer:
    ...
    def _generate_recommendations(self, user_info, dishes, meal_type):        
        ...
        for i, dish in enumerate(dishes):
            ...
            # 生成推荐解释----todo
            explanation = []
            for nut in ["蛋白质", "脂肪", "碳水化合物"]:
                nut_val = nutrition.get(nut, 0)
                nut_min, nut_max = meal_needs.get(nut, (0, float('inf')))
                
                if nut_min > 0 and nut_max < float('inf'):
                    if nut_val > nut_max * 0.8:
                        explanation.append(f"{nut}含量偏高")
                    elif nut_val < nut_min * 0.8:
                        explanation.append(f"{nut}含量偏低")
            # reason----todo
            reason = f"{', '.join(explanation)}" if explanation else "符合营养需求"
...
def recommend_dishes():
    ...
    try:
        ...
        # print("获取到的菜品列表：", dish_list)

        
        # 获取餐别
        meal_type = json_data.get("meal_type", "午餐")
        
        # 生成推荐
        result = dish_optimizer.generate_recommendations(user_info, dish_list, meal_type)
