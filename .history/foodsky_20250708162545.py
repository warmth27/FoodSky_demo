...
class DishOptimizer:
    ...
    def optimize_dish_weights(self, user_needs, dishes):
        ...
        for nutrient in key_nutrients:
            # 获取用户需求营养范围
            min_val, max_val = user_needs[nutrient]
            # 计算每道菜在该营养下的营养值
            dish_nutrient_vals = [pulp.lpDot(weights, [nutri[nutrient] for nutri in dish_nutritions]) for dish in dishes]
            # 添加约束：每道菜的营养值在合理范围内
            prob += pulp.lpSum(dish_nutrient_vals) >= min_val, f"Min_{nutrient}"
            prob += pulp.lpSum(dish_nutrient_vals) <= max_val, f"Max_{nutrient}"
