...
class DishOptimizer:
    ...
    def calculate_meal_needs(self, user_info, meal_type):
        ...
        for nutrient, value in daily_needs.items():
            if isinstance(value, (int, float)):
                ...
                meal_needs[nutrient] = (low * meal_ratio * 0.9, high * meal_ratio)
            else:
    ...
    def optimize_dish_weights(self, user_needs, dishes):
        ...
        for dish in dishes:
            ...
            # 使用关键营养素的平均值作为评分
            score = sum(nutrition.get(n, 0) for n in key_nutrients) / len(key_nutrients)
        ...
        for nutrient in key_nutrients:
            ...
            # 添加约束：加权总和在用户需求范围内
            if min_val > 0:
                prob += weighted_sum >= min_val * 0.9, f"{nutrient}_min"
            if max_val < float('inf'):
                prob += weighted_sum <= max_val * 1.1, f"{nutrient}_max"
