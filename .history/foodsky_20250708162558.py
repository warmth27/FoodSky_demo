...
class DishOptimizer:
    ...
    def optimize_dish_weights(self, user_needs, dishes):
        ...
        for nutrient in key_nutrients:
            ...
            # 计算营养范围
            min_range = sum(nutri[nutrient][0] for nutri in dish_nutritions)
            max_range = sum(nutri[nutrient][1] for nutri in dish_nutritions)
            # 添加约束
            prob += min_range <= min_val, f"Min_{nutrient}"
            prob += max_range >= max_val, f"Max_{nutrient}"
