...
def generate_all_dishes_reasons(dishes_info, meal_needs, weights, user_info, meal_type):
    ...
    # 准备提示词
    prompt = f"""
你是一位专业的营养师，正在为用户设计一份完整的{meal_type}餐单。以下是餐单中的14道菜品信息，请为每道菜品生成专业的推荐或者不推荐理由：
...
## 任务要求
1. 结合用户基本信息，用户单餐营养需求，以及菜品信息列表，和每道菜的权重，判断每道菜是推荐食用还是不推荐食用
2. 结合权重解释原因（权重是系统根据营养均衡度与适配性评估得出的指标，越高表示越推荐，越低表示不推荐）
3. 最终所有推荐食用的菜品营养范围总和不能超过用户单餐营养需求

## 输出格式使用JSON格式输出
## 输出示例
{{
    "番茄炒蛋": "富含优质蛋白和维生素C，脂肪含量适中。推荐权重0.85，营养均衡且符合需求，建议作为午餐主菜搭配米饭食用。",
