...
class DishOptimizer:
    ...
    def calculate_meal_needs(self, user_info, meal_type):
        ...
        return meal_needs
    # 计算单道菜品的营养范围
    def calculate_dish_nutrition(self, dish_name):
        if dish_name in recipeDataset:
            recipe = recipeDataset[dish_name]
            main_ingredients = recipe['主食材']
            other_ingredients = recipe['辅料']

            main_nutrition, main_weight = ingredients_calculate_weight(main_ingredients)
            other_nutrition, other_weight = ingredients_calculate_weight(other_ingredients)

            # 合并营养信息
            combined_nutrition = {}
            all_keys = set(main_nutrition.keys()) | set(other_nutrition.keys())
            for key in all_keys:
                main_value = float(main_nutrition.get(key, 0))
