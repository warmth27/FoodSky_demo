...
class DishOptimizer:
    ...
    def calculate_meal_needs(self, user_info, meal_type):
        ...
        for nutrient, value in daily_needs.items():
            if isinstance(value, (int, float)):
                ...
                meal_needs[nutrient] = (low * meal_ratio, high * meal_ratio * 1.1)
            else:
    ...
    def _generate_recommendations(self, user_info, dishes, meal_type):        
        ...
        for i, dish in enumerate(dishes):
            ...
            reason = f"推荐理由：{', '.join(explanation)}" if explanation else "推荐理由：符合营养需求"
...
def recommend_dishes():
    ...
    try:
        ...
        if len(dishes) != 14:
            return jsonify({"success": False, "error": "需要提供14道菜品"}), 400
        
        # 获取餐别
        meal_type = json_data.get("meal_type", "午餐")
        
        # 生成推荐
        result = dish_optimizer.generate_recommendations(user_info, dishes, meal_type)
